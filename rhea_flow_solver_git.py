...
name_restart    = 'restart_data_file.csv'  # Name of restart data [-]
######################################################################
...
R2_R1           = 0.01  # Chamber-convergent arc ratio as (R2/R1)
...
# Coordinates origin
# x_0 = xc
x_0 = -2.5  # Domain origin in x-direction [m]
...
def initialize_uvwPT(u, v, w, P, T, grid):
    # Create a linear pressure drop to prevent boundary explosions
...
def update_boundaries(sos, rho, rhou, rhov, rhow, rhoE, u, v, w, P, T, grid):
    ...
    # West boundary points (SUBSONIC INFLOW - PRESSURE RESERVOIR)
    i = 0
    for j in range(1, num_grid_y + 1):
        for k in range(1, num_grid_z + 1):
            ...
            # 1. Float the axial velocity (Zero-gradient extrapolation)
            # The nozzle will "suck" exactly as much fluid as it needs to choke.
            u_g = u_in 
            
            # 2. Force the flow to enter perfectly straight
            v_g = 0.0
            w_g = 0.0

            # 3. Hard-fix the ghost cells to the reservoir thermodynamics
            # (This acts as an infinite tank of gas at P_ref and T_ref)
            P_g = P_ref
            T_g = T_ref
    ...
    # East boundary points (SUBSONIC / SUPERSONIC OUTFLOW)
    i = num_grid_x + 1
    ...
    for j in range(1, num_grid_y + 1):
        for k in range(1, num_grid_z + 1):
            ...
            # Local Mach number normal to the boundary
            Ma_in = u_in / sos_in

            # Always extrapolate velocities and temperature (Zero-Gradient)
            u_g = u_in
            v_g = v_in
            w_g = w_in
            T_g = T_in

            # Switch pressure based on local flow regime
            if Ma_in < 1.0:
                # Subsonic Outflow: Impose physical back-pressure to drive flow
                P_g = P_exit
            else:
                # Supersonic Outflow: Flow is too fast for back-pressure to communicate
                P_g = P_in

            # Recalculate density and energy based on the boundary state
            rho_g = -1.0; e_g = -1.0
            rho_g, e_g = thermodynamics.calculateDensityInternalEnergyFromPressureTemperature(rho_g, e_g, P_g, T_g)
            
            ke_g = 0.5 * (u_g ** 2.0 + v_g ** 2.0 + w_g ** 2.0)
            E_g = e_g + ke_g
    ...
    # Ma_in = 0.1             # Uncomment to activate subsonic outflow only
    ...
    for i in range(1, num_grid_x + 1): # Original line: (for i in range(1, num_grid_x + 1):)
        for k in range(1, num_grid_z + 1): # Original line: (for k in range(1, num_grid_z + 1):)
            ...
            P_g = P_in  # Neumann - Zero gradient 
            T_g = T_in  # Neumann - Zero gradient
...
def inviscid_fluxes(rho_inv, rhou_inv, rhov_inv, rhow_inv, rhoE_inv, rho, u, v, w, E, s, P, P_thermo, T, sos, grid):
    # Strong Conservation Form using Face Normals
    
    # Pre-allocate face fluxes
    E_f_rho = np.zeros_like(rho_inv); E_f_rhou = np.zeros_like(rhou_inv)
    E_f_rhov = np.zeros_like(rhov_inv); E_f_rhow = np.zeros_like(rhow_inv)
    E_f_rhoE = np.zeros_like(rhoE_inv)
    
    F_f_rho = np.zeros_like(rho_inv); F_f_rhou = np.zeros_like(rhou_inv)
    F_f_rhov = np.zeros_like(rhov_inv); F_f_rhow = np.zeros_like(rhow_inv)
    F_f_rhoE = np.zeros_like(rhoE_inv)
    
    G_f_rho = np.zeros_like(rho_inv); G_f_rhou = np.zeros_like(rhou_inv)
    G_f_rhov = np.zeros_like(rhov_inv); G_f_rhow = np.zeros_like(rhow_inv)
    G_f_rhoE = np.zeros_like(rhoE_inv)
    
    # ---------------------------------------------------------
    # PASS 1: X-Faces (i + 1/2) 
    # ---------------------------------------------------------
    for i in range(0, num_grid_x + 1):
        for j in range(1, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                ...
                # Face geometry
                dy_face = 0.25 * (grid[i+1][j+1][k][1] + grid[i][j+1][k][1] - grid[i+1][j-1][k][1] - grid[i][j-1][k][1])
                dz_face = 0.5 * (grid[i][j][k+1][2] - grid[i][j][k-1][2])
                Area = dy_face * dz_face
                
                # X-face normal is perfectly straight (1, 0, 0)
                # No rotation needed, u is the normal velocity
                ...
                P_th_L = P_L - P_thermo;    P_th_R = P_R - P_thermo
                
                rho_f  = ECKEP_flux(rho_L, rho_R, u_L, u_R, v_L, v_R, w_L, w_R, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R, a_L, a_R, 0)
                rhou_f = ECKEP_flux(rho_L, rho_R, u_L, u_R, v_L, v_R, w_L, w_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 1)
                rhov_f = ECKEP_flux(rho_L, rho_R, u_L, u_R, v_L, v_R, w_L, w_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 2)
                rhow_f = ECKEP_flux(rho_L, rho_R, u_L, u_R, v_L, v_R, w_L, w_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 3)
                rhoE_f = ECKEP_flux(rho_L, rho_R, u_L, u_R, v_L, v_R, w_L, w_R, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R, a_L, a_R, 4)
                
                E_f_rho[i][j][k] = rho_f * Area
                E_f_rhou[i][j][k] = rhou_f * Area
                E_f_rhov[i][j][k] = rhov_f * Area
                E_f_rhow[i][j][k] = rhow_f * Area
                E_f_rhoE[i][j][k] = rhoE_f * Area

    # ---------------------------------------------------------
    # PASS 2: Y-Faces (j + 1/2) - The Curved Faces!
    # ---------------------------------------------------------
    for i in range(1, num_grid_x + 1):
        for j in range(0, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                index_L = j
                index_R = j + 1
                
                # Face geometry (Top curve of the cell)
                dx_face = 0.5 * (grid[i+1][j][k][0] - grid[i-1][j][k][0])
                y_right = 0.5 * (grid[i+1][j+1][k][1] + grid[i+1][j][k][1])
                y_left  = 0.5 * (grid[i-1][j+1][k][1] + grid[i-1][j][k][1])
                dy_face = y_right - y_left
                dz_face = 0.5 * (grid[i][j][k+1][2] - grid[i][j][k-1][2])
                
                # Normal vector to the curved surface
                norm_mag = np.sqrt(dx_face**2 + dy_face**2)
                n_x = -dy_face / norm_mag
                n_y = dx_face / norm_mag
                Area = norm_mag * dz_face
                ...
                # Rotate velocities to Face-Normal frame
                V_n_L = u_L * n_x + v_L * n_y
                V_t_L = -u_L * n_y + v_L * n_x
                V_n_R = u_R * n_x + v_R * n_y
                V_t_R = -u_R * n_y + v_R * n_x
                
                # Riemann Solver using Normal/Tangential velocities
                rho_fn  = ECKEP_flux(rho_L, rho_R, V_n_L, V_n_R, V_t_L, V_t_R, w_L, w_R, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R, a_L, a_R, 0)
                rhou_fn = ECKEP_flux(rho_L, rho_R, V_n_L, V_n_R, V_t_L, V_t_R, w_L, w_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 1) # Normal Momentum
                rhov_fn = ECKEP_flux(rho_L, rho_R, V_n_L, V_n_R, V_t_L, V_t_R, w_L, w_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 2) # Tangential Momentum
                rhow_fn = ECKEP_flux(rho_L, rho_R, V_n_L, V_n_R, V_t_L, V_t_R, w_L, w_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 3)
                rhoE_fn = ECKEP_flux(rho_L, rho_R, V_n_L, V_n_R, V_t_L, V_t_R, w_L, w_R, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R, a_L, a_R, 4)
                
                # Rotate momentum fluxes back to Cartesian and apply Area
                F_f_rho[i][j][k] = rho_fn * Area
                F_f_rhou[i][j][k] = (rhou_fn * n_x - rhov_fn * n_y) * Area
                F_f_rhov[i][j][k] = (rhou_fn * n_y + rhov_fn * n_x) * Area
                F_f_rhow[i][j][k] = rhow_fn * Area
                F_f_rhoE[i][j][k] = rhoE_fn * Area

    # ---------------------------------------------------------
    # PASS 3: Z-Faces (k + 1/2) 
    # ---------------------------------------------------------
    for i in range(1, num_grid_x + 1):
        for j in range(1, num_grid_y + 1):
            for k in range(0, num_grid_z + 1):
                index_L = k
                index_R = k + 1
                
                dx_face = 0.5 * (grid[i+1][j][k][0] - grid[i-1][j][k][0])
                dy_face = 0.5 * (grid[i][j+1][k][1] - grid[i][j-1][k][1])
                Area = dx_face * dy_face
                ...
                # Z face normal is (0,0,1). Pass w as the normal velocity.
                rho_fn  = ECKEP_flux(rho_L, rho_R, w_L, w_R, v_L, v_R, u_L, u_R, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R, a_L, a_R, 0)
                rhow_fn = ECKEP_flux(rho_L, rho_R, w_L, w_R, v_L, v_R, u_L, u_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 1) # Normal (Z)
                rhov_fn = ECKEP_flux(rho_L, rho_R, w_L, w_R, v_L, v_R, u_L, u_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 2) # Tangential (Y)
                rhou_fn = ECKEP_flux(rho_L, rho_R, w_L, w_R, v_L, v_R, u_L, u_R, E_L, E_R, s_L, s_R, P_th_L, P_th_R, T_L, T_R, a_L, a_R, 3) # Tangential (X)
                rhoE_fn = ECKEP_flux(rho_L, rho_R, w_L, w_R, v_L, v_R, u_L, u_R, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R, a_L, a_R, 4)
                
                G_f_rho[i][j][k] = rho_fn * Area
                G_f_rhou[i][j][k] = rhou_fn * Area
                G_f_rhov[i][j][k] = rhov_fn * Area
                G_f_rhow[i][j][k] = rhow_fn * Area
                G_f_rhoE[i][j][k] = rhoE_fn * Area

    # ---------------------------------------------------------
    # PASS 4: Update Cell Center (Exact Conservation)
    # ---------------------------------------------------------
    for i in range(1, num_grid_x + 1):
        for j in range(1, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                dx = 0.5 * (grid[i+1][j][k][0] - grid[i-1][j][k][0])
                dy = 0.5 * (grid[i][j+1][k][1] - grid[i][j-1][k][1])
                dz = 0.5 * (grid[i][j][k+1][2] - grid[i][j][k-1][2])
                Volume = dx * dy * dz
                
                # Flux Out - Flux In divided by Volume
                rho_inv[i][j][k] = ( (E_f_rho[i][j][k] - E_f_rho[i-1][j][k]) + 
                                     (F_f_rho[i][j][k] - F_f_rho[i][j-1][k]) + 
                                     (G_f_rho[i][j][k] - G_f_rho[i][j][k-1]) ) / Volume
                                     
                rhou_inv[i][j][k] = ( (E_f_rhou[i][j][k] - E_f_rhou[i-1][j][k]) + 
                                      (F_f_rhou[i][j][k] - F_f_rhou[i][j-1][k]) + 
                                      (G_f_rhou[i][j][k] - G_f_rhou[i][j][k-1]) ) / Volume
                                      
                rhov_inv[i][j][k] = ( (E_f_rhov[i][j][k] - E_f_rhov[i-1][j][k]) + 
                                      (F_f_rhov[i][j][k] - F_f_rhov[i][j-1][k]) + 
                                      (G_f_rhov[i][j][k] - G_f_rhov[i][j][k-1]) ) / Volume
                                      
                rhow_inv[i][j][k] = ( (E_f_rhow[i][j][k] - E_f_rhow[i-1][j][k]) + 
                                      (F_f_rhow[i][j][k] - F_f_rhow[i][j-1][k]) + 
                                      (G_f_rhow[i][j][k] - G_f_rhow[i][j][k-1]) ) / Volume
                                      
                rhoE_inv[i][j][k] = ( (E_f_rhoE[i][j][k] - E_f_rhoE[i-1][j][k]) + 
                                      (F_f_rhoE[i][j][k] - F_f_rhoE[i][j-1][k]) + 
                                      (G_f_rhoE[i][j][k] - G_f_rhoE[i][j][k-1]) ) / Volume
    # print( rho_inv )
    # print( rhou_inv )
    # print( rhov_inv )
    # print( rhow_inv )
...
@njit
def viscous_fluxes(rhou_vis, rhov_vis, rhow_vis, rhoE_vis, work_vis_rhoe, u, v, w, T, mu, kappa, grid, L_y, drc_dx):
    # Two-Pass Conservative Method for Viscous Fluxes
    
    # Pre-allocate face flux arrays
    E_v_rhou = np.zeros_like(rhou_vis); E_v_rhov = np.zeros_like(rhov_vis)
    E_v_rhow = np.zeros_like(rhow_vis); E_v_rhoE = np.zeros_like(rhoE_vis)
    
    F_v_rhou = np.zeros_like(rhou_vis); F_v_rhov = np.zeros_like(rhov_vis)
    F_v_rhow = np.zeros_like(rhow_vis); F_v_rhoE = np.zeros_like(rhoE_vis)
    
    G_v_rhou = np.zeros_like(rhou_vis); G_v_rhov = np.zeros_like(rhov_vis)
    G_v_rhow = np.zeros_like(rhow_vis); G_v_rhoE = np.zeros_like(rhoE_vis)
    
    # ---------------------------------------------------------
    # PASS 1: Calculate Stresses at Cell Faces
    # ---------------------------------------------------------
    ...
    for i in range(1, num_grid_x + 1):
        for j in range(1, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                ...
                # 1A. X-Faces (i + 1/2)
                # ---------------------------------------------------------
                delta_x = grid[i+1][j][k][0] - grid[i][j][k][0]
                delta_y = 0.5 * (grid[i][j+1][k][1] - grid[i][j-1][k][1])
                delta_z = 0.5 * (grid[i][j][k+1][2] - grid[i][j][k-1][2])
                
                # Averages at the face
                mu_face = 0.5 * (mu[i+1][j][k] + mu[i][j][k])
                kappa_face = 0.5 * (kappa[i+1][j][k] + kappa[i][j][k])
                u_face = 0.5 * (u[i+1][j][k] + u[i][j][k])
                v_face = 0.5 * (v[i+1][j][k] + v[i][j][k])
                w_face = 0.5 * (w[i+1][j][k] + w[i][j][k])
                
                # Metric term at the face (averaged from i and i+1)
                y_pos_face = 0.5 * (grid[i+1][j][k][1] + grid[i][j][k][1])
                L_y_face = 0.5 * (L_y[i+1] + L_y[i])
                drc_dx_face = 0.5 * (drc_dx[i+1] + drc_dx[i])
                d_N_d_x_face = -1.0 * (y_pos_face / L_y_face) * drc_dx_face
                
                # Straight computational derivatives
                d_u_xi = (u[i+1][j][k] - u[i][j][k]) / delta_x
                d_v_xi = (v[i+1][j][k] - v[i][j][k]) / delta_x
                d_w_xi = (w[i+1][j][k] - w[i][j][k]) / delta_x
                d_T_xi = (T[i+1][j][k] - T[i][j][k]) / delta_x
                
                # Transverse derivatives (averaged)
                d_u_eta = 0.5 * ( (u[i+1][j+1][k] - u[i+1][j-1][k])/(2.0*delta_y) + (u[i][j+1][k] - u[i][j-1][k])/(2.0*delta_y) )
                d_v_eta = 0.5 * ( (v[i+1][j+1][k] - v[i+1][j-1][k])/(2.0*delta_y) + (v[i][j+1][k] - v[i][j-1][k])/(2.0*delta_y) )
                d_w_eta = 0.5 * ( (w[i+1][j+1][k] - w[i+1][j-1][k])/(2.0*delta_y) + (w[i][j+1][k] - w[i][j-1][k])/(2.0*delta_y) )
                d_T_eta = 0.5 * ( (T[i+1][j+1][k] - T[i+1][j-1][k])/(2.0*delta_y) + (T[i][j+1][k] - T[i][j-1][k])/(2.0*delta_y) )
                
                d_u_z = 0.5 * ( (u[i+1][j][k+1] - u[i+1][j][k-1])/(2.0*delta_z) + (u[i][j][k+1] - u[i][j][k-1])/(2.0*delta_z) )
                d_v_z = 0.5 * ( (v[i+1][j][k+1] - v[i+1][j][k-1])/(2.0*delta_z) + (v[i][j][k+1] - v[i][j][k-1])/(2.0*delta_z) )
                d_w_z = 0.5 * ( (w[i+1][j][k+1] - w[i+1][j][k-1])/(2.0*delta_z) + (w[i][j][k+1] - w[i][j][k-1])/(2.0*delta_z) )
                
                # True physical derivatives (applying the metric chain rule)
                d_u_x = d_u_xi + d_N_d_x_face * d_u_eta
                d_v_x = d_v_xi + d_N_d_x_face * d_v_eta
                d_w_x = d_w_xi + d_N_d_x_face * d_w_eta
                d_T_x = d_T_xi + d_N_d_x_face * d_T_eta
                
                d_u_y = d_u_eta * (1.0 / L_y_face)
                d_v_y = d_v_eta * (1.0 / L_y_face)
                d_w_y = d_w_eta * (1.0 / L_y_face)
                d_T_y = d_T_eta * (1.0 / L_y_face)

                # Divergence and Stresses
                div_uvw = d_u_x + d_v_y + d_w_z
                tau_xx = 2.0 * mu_face * (d_u_x - (div_uvw / 3.0))
                tau_xy = mu_face * (d_u_y + d_v_x)
                tau_xz = mu_face * (d_u_z + d_w_x)
                q_x = -kappa_face * d_T_x
                
                E_v_rhou[i][j][k] = tau_xx
                E_v_rhov[i][j][k] = tau_xy
                E_v_rhow[i][j][k] = tau_xz
                E_v_rhoE[i][j][k] = u_face*tau_xx + v_face*tau_xy + w_face*tau_xz - q_x

                # ---------------------------------------------------------
                # 1B. Y-Faces (j + 1/2)
                # ---------------------------------------------------------
                delta_y_face = grid[i][j+1][k][1] - grid[i][j][k][1]
                
                mu_face_y = 0.5 * (mu[i][j+1][k] + mu[i][j][k])
                kappa_face_y = 0.5 * (kappa[i][j+1][k] + kappa[i][j][k])
                u_face_y = 0.5 * (u[i][j+1][k] + u[i][j][k])
                v_face_y = 0.5 * (v[i][j+1][k] + v[i][j][k])
                w_face_y = 0.5 * (w[i][j+1][k] + w[i][j][k])
                
                # Y-face metrics
                y_pos_face_y = 0.5 * (grid[i][j+1][k][1] + grid[i][j][k][1])
                d_N_d_x_face_y = -1.0 * (y_pos_face_y / L_y[i]) * drc_dx[i]
                
                d_u_eta_y = (u[i][j+1][k] - u[i][j][k]) / delta_y_face
                d_v_eta_y = (v[i][j+1][k] - v[i][j][k]) / delta_y_face
                d_w_eta_y = (w[i][j+1][k] - w[i][j][k]) / delta_y_face
                d_T_eta_y = (T[i][j+1][k] - T[i][j][k]) / delta_y_face
                
                d_u_xi_y = 0.5 * ( (u[i+1][j+1][k] - u[i-1][j+1][k])/(2.0*delta_x) + (u[i+1][j][k] - u[i-1][j][k])/(2.0*delta_x) )
                d_v_xi_y = 0.5 * ( (v[i+1][j+1][k] - v[i-1][j+1][k])/(2.0*delta_x) + (v[i+1][j][k] - v[i-1][j][k])/(2.0*delta_x) )
                d_w_xi_y = 0.5 * ( (w[i+1][j+1][k] - w[i-1][j+1][k])/(2.0*delta_x) + (w[i+1][j][k] - w[i-1][j][k])/(2.0*delta_x) )
                
                d_u_z_y = 0.5 * ( (u[i][j+1][k+1] - u[i][j+1][k-1])/(2.0*delta_z) + (u[i][j][k+1] - u[i][j][k-1])/(2.0*delta_z) )
                d_v_z_y = 0.5 * ( (v[i][j+1][k+1] - v[i][j+1][k-1])/(2.0*delta_z) + (v[i][j][k+1] - v[i][j][k-1])/(2.0*delta_z) )
                d_w_z_y = 0.5 * ( (w[i][j+1][k+1] - w[i][j+1][k-1])/(2.0*delta_z) + (w[i][j][k+1] - w[i][j][k-1])/(2.0*delta_z) )

                d_u_x_y = d_u_xi_y + d_N_d_x_face_y * d_u_eta_y
                d_v_x_y = d_v_xi_y + d_N_d_x_face_y * d_v_eta_y
                d_w_x_y = d_w_xi_y + d_N_d_x_face_y * d_w_eta_y

                d_u_y_y = d_u_eta_y * (1.0 / L_y[i])
                d_v_y_y = d_v_eta_y * (1.0 / L_y[i])
                d_w_y_y = d_w_eta_y * (1.0 / L_y[i])
                d_T_y_y = d_T_eta_y * (1.0 / L_y[i])
                
                div_uvw_y = d_u_x_y + d_v_y_y + d_w_z_y
                tau_yy_y = 2.0 * mu_face_y * (d_v_y_y - (div_uvw_y / 3.0))
                tau_xy_y = mu_face_y * (d_u_y_y + d_v_x_y)
                tau_yz_y = mu_face_y * (d_v_z_y + d_w_y_y)
                q_y_y = -kappa_face_y * d_T_y_y
                
                F_v_rhou[i][j][k] = tau_xy_y
                F_v_rhov[i][j][k] = tau_yy_y
                F_v_rhow[i][j][k] = tau_yz_y
                F_v_rhoE[i][j][k] = u_face_y*tau_xy_y + v_face_y*tau_yy_y + w_face_y*tau_yz_y - q_y_y

                # ---------------------------------------------------------
                # 1C. Z-Faces (k + 1/2) (Simplified assuming uniform Z)
                # ---------------------------------------------------------
                delta_z_face = grid[i][j][k+1][2] - grid[i][j][k][2]
                
                mu_face_z = 0.5 * (mu[i][j][k+1] + mu[i][j][k])
                kappa_face_z = 0.5 * (kappa[i][j][k+1] + kappa[i][j][k])
                u_face_z = 0.5 * (u[i][j][k+1] + u[i][j][k])
                v_face_z = 0.5 * (v[i][j][k+1] + v[i][j][k])
                w_face_z = 0.5 * (w[i][j][k+1] + w[i][j][k])
                
                # Z-face metrics
                y_pos_face_z = 0.5 * (grid[i][j][k+1][1] + grid[i][j][k][1])
                d_N_d_x_face_z = -1.0 * (y_pos_face_z / L_y[i]) * drc_dx[i]
                
                d_u_z_z = (u[i][j][k+1] - u[i][j][k]) / delta_z_face
                d_v_z_z = (v[i][j][k+1] - v[i][j][k]) / delta_z_face
                d_w_z_z = (w[i][j][k+1] - w[i][j][k]) / delta_z_face
                d_T_z_z = (T[i][j][k+1] - T[i][j][k]) / delta_z_face
                
                d_u_xi_z = 0.5 * ( (u[i+1][j][k+1] - u[i-1][j][k+1])/(2.0*delta_x) + (u[i+1][j][k] - u[i-1][j][k])/(2.0*delta_x) )
                d_v_xi_z = 0.5 * ( (v[i+1][j][k+1] - v[i-1][j][k+1])/(2.0*delta_x) + (v[i+1][j][k] - v[i-1][j][k])/(2.0*delta_x) )
                d_w_xi_z = 0.5 * ( (w[i+1][j][k+1] - w[i-1][j][k+1])/(2.0*delta_x) + (w[i+1][j][k] - w[i-1][j][k])/(2.0*delta_x) )

                d_u_eta_z = 0.5 * ( (u[i][j+1][k+1] - u[i][j-1][k+1])/(2.0*delta_y) + (u[i][j+1][k] - u[i][j-1][k])/(2.0*delta_y) )
                d_v_eta_z = 0.5 * ( (v[i][j+1][k+1] - v[i][j-1][k+1])/(2.0*delta_y) + (v[i][j+1][k] - v[i][j-1][k])/(2.0*delta_y) )
                d_w_eta_z = 0.5 * ( (w[i][j+1][k+1] - w[i][j-1][k+1])/(2.0*delta_y) + (w[i][j+1][k] - w[i][j-1][k])/(2.0*delta_y) )

                # Use the properly defined Z-face metric here
                d_u_x_z = d_u_xi_z + d_N_d_x_face_z * d_u_eta_z
                d_v_x_z = d_v_xi_z + d_N_d_x_face_z * d_v_eta_z
                d_w_x_z = d_w_xi_z + d_N_d_x_face_z * d_w_eta_z
                
                d_u_y_z = d_u_eta_z * (1.0 / L_y[i])
                d_v_y_z = d_v_eta_z * (1.0 / L_y[i])
                d_w_y_z = d_w_eta_z * (1.0 / L_y[i])
                
                div_uvw_z = d_u_x_z + d_v_y_z + d_w_z_z
                tau_zz_z = 2.0 * mu_face_z * (d_w_z_z - (div_uvw_z / 3.0))
                tau_xz_z = mu_face_z * (d_u_z_z + d_w_x_z)
                tau_yz_z = mu_face_z * (d_v_z_z + d_w_y_z)
                q_z_z = -kappa_face_z * d_T_z_z
                
                G_v_rhou[i][j][k] = tau_xz_z
                G_v_rhov[i][j][k] = tau_yz_z
                G_v_rhow[i][j][k] = tau_zz_z
                G_v_rhoE[i][j][k] = u_face_z*tau_xz_z + v_face_z*tau_yz_z + w_face_z*tau_zz_z - q_z_z
    # ---------------------------------------------------------
    # PASS 2: Cell-Centered Divergence
    # ---------------------------------------------------------
    for i in range(1, num_grid_x + 1):
        for j in range(1, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                
                # Physical Distances across the cell
                dx = 0.5 * (grid[i+1][j][k][0] - grid[i-1][j][k][0])
                dy = 0.5 * (grid[i][j+1][k][1] - grid[i][j-1][k][1])
                dz = 0.5 * (grid[i][j][k+1][2] - grid[i][j][k-1][2])
                
                # Divergence is exactly the face flux differences
                rhou_vis[i][j][k] = (E_v_rhou[i][j][k] - E_v_rhou[i-1][j][k]) / dx + \
                                    (F_v_rhou[i][j][k] - F_v_rhou[i][j-1][k]) / dy + \
                                    (G_v_rhou[i][j][k] - G_v_rhou[i][j][k-1]) / dz
                                    
                rhov_vis[i][j][k] = (E_v_rhov[i][j][k] - E_v_rhov[i-1][j][k]) / dx + \
                                    (F_v_rhov[i][j][k] - F_v_rhov[i][j-1][k]) / dy + \
                                    (G_v_rhov[i][j][k] - G_v_rhov[i][j][k-1]) / dz
                                    
                rhow_vis[i][j][k] = (E_v_rhow[i][j][k] - E_v_rhow[i-1][j][k]) / dx + \
                                    (F_v_rhow[i][j][k] - F_v_rhow[i][j-1][k]) / dy + \
                                    (G_v_rhow[i][j][k] - G_v_rhow[i][j][k-1]) / dz
                                    
                rhoE_vis[i][j][k] = (E_v_rhoE[i][j][k] - E_v_rhoE[i-1][j][k]) / dx + \
                                    (F_v_rhoE[i][j][k] - F_v_rhoE[i][j-1][k]) / dy + \
                                    (G_v_rhoE[i][j][k] - G_v_rhoE[i][j][k-1]) / dz
                                    
                # Work of viscous stresses on internal energy (removing q components)
                # (You can calculate this by re-evaluating the central q divergence, 
                # or just storing q fluxes separately in Pass 1. 
                # For brevity, assuming work_vis_rhoe handles just mechanical work)
                work_vis_rhoe[i][j][k] = rhoE_vis[i][j][k] # Placeholder: subtract divergence of q here
    # print( rhou_vis )
    # print( rhov_vis )
    # print( rhow_vis )
    # print( rhoE_vis )
...
#################################################################
plt.figure()
plt.scatter(physical_plane[:,:,1,0],physical_plane[:,:,1,1],s=2)
plt.figure()
plt.scatter(computational_plane[:,:,1,0],computational_plane[:,:,1,1],s=2)
plt.show()
# exit()
#################################################################

### Differentiate physical contour
j = 0; k = 0
for i in range(0,num_grid_x+2):
...
while time_iter < max_num_time_iter:
   ...
   ### Runge-Kutta sub-steps
    for rk in range( 0, rk_order ):
        ...
        ### Calculate viscous fluxes
        viscous_fluxes(rhou_vis_flux, rhov_vis_flux, rhow_vis_flux, rhoE_vis_flux, work_vis_rhoe_flux, u_field, v_field,
                       w_field, T_field, mu_field, kappa_field, computational_plane, L_y, drc_dx)
