...
name_restart    = 'output_data/output_data_3000.csv'  # Name of restart data [-]
######################################################################

###################### DATA OUTPUT SETTINGS ##########################
name_file_out   = 'output_data_test/output_data'  # Name of output data [-]
...
P_exit          = 200.0e5  # Subsonic outlet exit pressure [Pa]
...
max_subr_iter       = 1000  # Maximum number of iterations for subsonic inlet subroutine
...
# x_0 = xc - L_c
# x_0 = xc
x_0 = 0.0  # Domain origin in x-direction (Pipe) [m]
    ...
    for i in range(0, num_grid_x + 2):
        for j in range(0, num_grid_y + 2):
            for k in range(0, num_grid_z + 2):
                ...
                rc = 0.5 * (physical_plane[i][-1][k][1] + physical_plane[i][-2][k][1])
                rc_in = 0.5 * (physical_plane[1][-1][k][1] + physical_plane[1][-2][k][1])
                r_normalized = min(abs(physical_plane[i][j][k][1])/rc, 1.0)

                # rc = physical_plane[i][-1][k][1]
                # rc_in = physical_plane[0][-1][k][1] 
                u_core = u_inlet #* rc_in/rc  # rc_in/rc is accelerating and decelerating the flow

                # u[i][j][k] = u_core * (1.0 - r_normalized)**(1.0/7.0)
                # v[i][j][k] = v_inlet
                # w[i][j][k] = w_inlet
                P[i][j][k] = P_ref
                T[i][j][k] = T_ref
...
def update_boundaries(sos, rho, rhou, rhov, rhow, rhoE, u, v, w, P, T, grid):
    ...
    for j in range(1, num_grid_y + 1):
        for k in range(1, num_grid_z + 1): # Original line (for k in range(1, num_grid_z + 1):)
            ...
            # rc = 0.5 * (physical_plane[i][-1][k][1] + physical_plane[i][-2][k][1])
            # u_inflow = - (u_inlet/(rc)**2)*physical_plane[i][j][k][1]**2 + u_inlet
            ...
            # V_g = np.sqrt(u_g**2 + v_g**2 + w_g**2)
            # P_static = P_ref - 0.5 * rho_g * V_g**2
            # P_g = (P_static - wg_in * P_in) / wg_g
            # T_g = (T_ref - wg_in * T_in) / wg_g
    ...
    u_axis = 0.5 * (u[-2, 1, 1] + u[-2, 1, 1])
    v_axis = 0.5 * (v[-2, 1, 1] + v[-2, 1, 1])
    w_axis = 0.5 * (w[-2, 1, 1] + w[-2, 1, 1])
    sos_axis = 0.5 * (sos[-2, 1, 1] + sos[-2, 1, 1])
    ...
    for i in range(1, num_grid_x + 1):
        for j in range(1, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                ...
                # rho
                var_type = 0
                rho_F_p_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_p, V_n_xi_R_p, V_t1_xi_L_p, V_t1_xi_R_p, V_t2_xi_L_p, V_t2_xi_R_p, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R,
                                     a_L, a_R, var_type)
                # rhou
                var_type = 1
                rho_n_F_p_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_p, V_n_xi_R_p, V_t1_xi_L_p, V_t1_xi_R_p, V_t2_xi_L_p, V_t2_xi_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhov
                var_type = 2
                rho_t1_F_p_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_p, V_n_xi_R_p, V_t1_xi_L_p, V_t1_xi_R_p, V_t2_xi_L_p, V_t2_xi_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhow
                var_type = 3
                rho_t2_F_p_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_p, V_n_xi_R_p, V_t1_xi_L_p, V_t1_xi_R_p, V_t2_xi_L_p, V_t2_xi_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhoE
                var_type = 4
                rhoE_F_p_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_p, V_n_xi_R_p, V_t1_xi_L_p, V_t1_xi_R_p, V_t2_xi_L_p, V_t2_xi_R_p, E_L, E_R, s_L, s_R, P_L, P_R, T_L,
                ...
                # rho
                var_type = 0
                rho_F_m_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_m, V_n_xi_R_m, V_t1_xi_L_m, V_t1_xi_R_m, V_t2_xi_L_m, V_t2_xi_R_m, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R,
                                     a_L, a_R, var_type)
                # rhou
                var_type = 1
                rho_n_F_m_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_m, V_n_xi_R_m, V_t1_xi_L_m, V_t1_xi_R_m, V_t2_xi_L_m, V_t2_xi_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhov
                var_type = 2
                rho_t1_F_m_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_m, V_n_xi_R_m, V_t1_xi_L_m, V_t1_xi_R_m, V_t2_xi_L_m, V_t2_xi_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhow
                var_type = 3
                rho_t2_F_m_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_m, V_n_xi_R_m, V_t1_xi_L_m, V_t1_xi_R_m, V_t2_xi_L_m, V_t2_xi_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhoE
                var_type = 4
                rhoE_F_m_x = ECKEP_flux(rho_L, rho_R, V_n_xi_L_m, V_n_xi_R_m, V_t1_xi_L_m, V_t1_xi_R_m, V_t2_xi_L_m, V_t2_xi_R_m, E_L, E_R, s_L, s_R, P_L, P_R, T_L,
                                      T_R, a_L, a_R, var_type)
                ...
                # rho
                var_type = 0
                rho_F_p_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_p, V_n_eta_R_p, V_t1_eta_L_p, V_t1_eta_R_p, V_t2_eta_L_p, V_t2_eta_R_p, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R,
                                     a_L, a_R, var_type)
                # rhou
                var_type = 1
                rho_n_F_p_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_p, V_n_eta_R_p, V_t1_eta_L_p, V_t1_eta_R_p, V_t2_eta_L_p, V_t2_eta_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhov
                var_type = 2
                rho_t1_F_p_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_p, V_n_eta_R_p, V_t1_eta_L_p, V_t1_eta_R_p, V_t2_eta_L_p, V_t2_eta_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhow
                var_type = 3
                rho_t2_F_p_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_p, V_n_eta_R_p, V_t1_eta_L_p, V_t1_eta_R_p, V_t2_eta_L_p, V_t2_eta_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhoE
                var_type = 4
                rhoE_F_p_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_p, V_n_eta_R_p, V_t1_eta_L_p, V_t1_eta_R_p, V_t2_eta_L_p, V_t2_eta_R_p, E_L, E_R, s_L, s_R, P_L, P_R, T_L,
                                      T_R, a_L, a_R, var_type)
                ...
                # rho
                var_type = 0
                rho_F_m_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_m, V_n_eta_R_m, V_t1_eta_L_m, V_t1_eta_R_m, V_t2_eta_L_m, V_t2_eta_R_m, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R,
                                     a_L, a_R, var_type)
                # rhou
                var_type = 1
                rho_n_F_m_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_m, V_n_eta_R_m, V_t1_eta_L_m, V_t1_eta_R_m, V_t2_eta_L_m, V_t2_eta_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhov
                var_type = 2
                rho_t1_F_m_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_m, V_n_eta_R_m, V_t1_eta_L_m, V_t1_eta_R_m, V_t2_eta_L_m, V_t2_eta_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhow
                var_type = 3
                rho_t2_F_m_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_m, V_n_eta_R_m, V_t1_eta_L_m, V_t1_eta_R_m, V_t2_eta_L_m, V_t2_eta_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhoE
                var_type = 4
                rhoE_F_m_y = ECKEP_flux(rho_L, rho_R, V_n_eta_L_m, V_n_eta_R_m, V_t1_eta_L_m, V_t1_eta_R_m, V_t2_eta_L_m, V_t2_eta_R_m, E_L, E_R, s_L, s_R, P_L, P_R, T_L,
                                      T_R, a_L, a_R, var_type)
                ...
                # rho
                var_type = 0
                rho_F_p_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_p, V_n_zeta_R_p, V_t1_zeta_L_p, V_t1_zeta_R_p, V_t2_zeta_L_p, V_t2_zeta_R_p, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R,
                                     a_L, a_R, var_type)
                # rhou
                var_type = 1
                rho_n_F_p_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_p, V_n_zeta_R_p, V_t1_zeta_L_p, V_t1_zeta_R_p, V_t2_zeta_L_p, V_t2_zeta_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhov
                var_type = 2
                rho_t1_F_p_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_p, V_n_zeta_R_p, V_t1_zeta_L_p, V_t1_zeta_R_p, V_t2_zeta_L_p, V_t2_zeta_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhow
                var_type = 3
                rho_t2_F_p_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_p, V_n_zeta_R_p, V_t1_zeta_L_p, V_t1_zeta_R_p, V_t2_zeta_L_p, V_t2_zeta_R_p, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhoE
                var_type = 4
                rhoE_F_p_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_p, V_n_zeta_R_p, V_t1_zeta_L_p, V_t1_zeta_R_p, V_t2_zeta_L_p, V_t2_zeta_R_p, E_L, E_R, s_L, s_R, P_L, P_R, T_L,
                                      T_R, a_L, a_R, var_type)
                ...
                # rho
                var_type = 0
                rho_F_m_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_m, V_n_zeta_R_m, V_t1_zeta_L_m, V_t1_zeta_R_m, V_t2_zeta_L_m, V_t2_zeta_R_m, E_L, E_R, s_L, s_R, P_L, P_R, T_L, T_R,
                                     a_L, a_R, var_type)
                # rhou
                var_type = 1
                rho_n_F_m_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_m, V_n_zeta_R_m, V_t1_zeta_L_m, V_t1_zeta_R_m, V_t2_zeta_L_m, V_t2_zeta_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhov
                var_type = 2
                rho_t1_F_m_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_m, V_n_zeta_R_m, V_t1_zeta_L_m, V_t1_zeta_R_m, V_t2_zeta_L_m, V_t2_zeta_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhow
                var_type = 3
                rho_t2_F_m_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_m, V_n_zeta_R_m, V_t1_zeta_L_m, V_t1_zeta_R_m, V_t2_zeta_L_m, V_t2_zeta_R_m, E_L, E_R, s_L, s_R, P_rhouvw_L,
                                      P_rhouvw_R, T_L, T_R, a_L, a_R, var_type)
                # rhoE
                var_type = 4
                rhoE_F_m_z = ECKEP_flux(rho_L, rho_R, V_n_zeta_L_m, V_n_zeta_R_m, V_t1_zeta_L_m, V_t1_zeta_R_m, V_t2_zeta_L_m, V_t2_zeta_R_m, E_L, E_R, s_L, s_R, P_L, P_R, T_L,
                                      T_R, a_L, a_R, var_type)
        ...
        # print( rho_inv[i][j][k] )
        # print( rhou_inv[i][j][k] )
        # print( rhov_inv[i][j][k] )
        # print( rhow_inv[i][j][k] )
        # print( rhoE_inv[i][j][k] )
...
ax1.scatter(physical_plane[:,:,1,0],physical_plane[:,:,1,1],s=2)
ax2.scatter(computational_plane[:,:,1,0],computational_plane[:,:,1,1],s=2)
plt.show()
# exit()
#################################################################
...
# j = 0; k = 1
# for i in range(0,num_grid_x+2):
#     drc_dx[i] = - 4.0 /(math.e**(computational_plane[i][j][k][0])+math.e**(-computational_plane[i][j][k][0]))**2 * 1.0/100.0
#     drc_dx_dx[i] = 8.0 * (math.e**(computational_plane[i][j][k][0])-math.e**(-computational_plane[i][j][k][0]))/(math.e**(computational_plane[i][j][k][0])+math.e**(-computational_plane[i][j][k][0]))**3 * 1.0/100.0 * 1.0/100.0
