...
name_restart    = 'output_data/output_data_29-05-2026/output_data_37500.csv'  # Name of restart data [-]
######################################################################

###################### DATA OUTPUT SETTINGS ##########################
name_file_out   = 'output_data/output_data_29-05-2026/output_data'  # Name of output data [-]
output_iter     = 500  # Output data every given number of iterations
######################################################################

######################## INITIAL CONDITIONS ##########################
u_inlet         = 1.0  # Subsonic inlet velocity in the x-direction [m/s]
v_inlet         = 0.0  # Subsonic inlet velocity in the y-direction [m/s]
w_inlet         = 0.0  # Subsonic inlet velocity in the z-direction [m/s]
T_ref           = 600.0  # Subsonic inlet temperature [K]
P_ref           = 2.0e5  # Subsonic inlet pressure [Pa]
P_exit          = 1.0e5  # Subsonic outlet exit pressure [Pa]
...
rt              = 0.8e-3  # Throat radius [m]
rc              = 2.0e-3  # Chamber radius [m]
R1_rt           = 20.0  # Convergent-throat arc ratio as (R1/rt)
R2_R1           = 7.0  # Chamber-convergent arc ratio as (R2/R1)
Rexp_rt         = 50.0  # Expansion arc ratio as (Rexp/rt)
...
L_c             = 5.0e-3 # Chamber section length [m]
...
mesh = np.zeros([num_grid_x + 2, num_grid_y + 2, num_grid_z + 2, num_sptl_dim])  # 3-D positions of the physical plane mesh
mesh = np.zeros([num_grid_x + 2, num_grid_y + 2, num_grid_z + 2, num_sptl_dim]) # 3-D positions of the computational plane grid
    ...
    for i in range(0, num_grid_x + 2):
        for j in range(0, num_grid_y + 2):
            for k in range(0, num_grid_z + 2):
                ...
                rc = 0.5 * (mesh[i][-1][k][1] + mesh[i][-2][k][1])
                rc_in = 0.5 * (mesh[1][-1][k][1] + mesh[1][-2][k][1])
                r_normalized = min(abs(mesh[i][j][k][1])/rc, 1.0) 

                # rc = mesh[i][-1][k][1]
                # rc_in = mesh[0][-1][k][1] 
                u_core = u_inlet * rc_in/rc
...
def update_boundaries(sos, rho, rhou, rhov, rhow, rhoE, u, v, w, P, T, grid):
    ...
    for j in range(1, num_grid_y + 1):
        for k in range(1, num_grid_z + 1): # Original line (for k in range(1, num_grid_z + 1):)
            ...
            # rc = 0.5 * (mesh[i][-1][k][1] + mesh[i][-2][k][1])
            # u_inflow = - (u_inlet/(rc)**2)*mesh[i][j][k][1]**2 + u_inlet
    ...
    if Ma_in < 0.99: 
        for j in range(1, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                ...
                sos_in = sos[i - 1][j][k]
                Ma_in = u_in / sos_in
                K_in = 0.5 * sos_in * (1.0 - Ma_in ** 2) / L_x
    ...
    for i in range(1, num_grid_x + 1):
        for j in range(1, num_grid_y + 1):
            for k in range(1, num_grid_z + 1):
                ...
                deta_dy = eta[i][j][k][1]
                deta_dz = eta[i][j][k][2]
                dzeta_dx = zeta[i][j][k][0]
                dzeta_dy = zeta[i][j][k][1]
                dzeta_dz = zeta[i][j][k][2]
...
# generate_computationalDomain(mesh, mesh)

# Uncomment segment to generate physical and computational grids plots
#################################################################
# fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10, 5))
# ax1.scatter(mesh[:,:,1,0],mesh[:,:,1,1],s=2)
# ax2.scatter(mesh[:,:,1,0],mesh[:,:,1,1],s=2)
# plt.show()
# exit()
#################################################################

### Differentiate physicial contour
# drc_dx = drc_dz_segments(drc_dx, mesh, R1, R2, Rexp, theta, alpha, x1, x2, xc, xexp) # (ADDED)
# print( drc_dx )
# j = 0; k = 1
# for i in range(0,num_grid_x+2):
#     drc_dx[i] = - 4.0 /(math.e**(mesh[i][j][k][0])+math.e**(-mesh[i][j][k][0]))**2 * 1.0/100.0
#     drc_dx_dx[i] = 8.0 * (math.e**(mesh[i][j][k][0])-math.e**(-mesh[i][j][k][0]))/(math.e**(mesh[i][j][k][0])+math.e**(-mesh[i][j][k][0]))**3 * 1.0/100.0 * 1.0/100.0
# drc_dx_dx = drc_dz_dz_segments(drc_dx_dx, mesh, R1, R2, Rexp, theta, alpha, x1, x2, xc, xexp) # (ADDED)

### Generate L_y vector in terms of x-distance
# L_y = np.zeros(num_grid_x+2)

# for i in range(0,num_grid_x+2):
#     L_y[i] = 0.5*(mesh[i][-1][1][1]+mesh[i][-2][1][1])
...
while time_iter < max_num_time_iter:
    ...
    inlet_data = ["Inlet south",  mesh[1,1,1,0], mesh[1,1,1,1],
                  ...
                  inlet_Ma]
    outlet_data = ["Outlet south",  mesh[-1,1,1,0], mesh[-1,1,1,1], 
                   ...
                   outlet_Ma]
    outlet_data_2 = ["Outlet north",  mesh[-1,-2,1,0], mesh[-1,-2,1,1], 
                   ...
                   outlet_Ma]
    inlet_data_2 = ["Inlet north",  mesh[1,-2,1,0], mesh[1,-2,1,1], 
                   P_field[1,-2,1] / 1e5,
    ...
    if (time_iter % output_iter == 0):
        ...
        # plt.figure()
        # plt.scatter(mesh[1:num_grid_x,1:num_grid_y,1,0],mesh[1:num_grid_x,1:num_grid_y,1,1],c=P_field[1:num_grid_x,1:num_grid_y,1],s=2)
        # plt.colorbar()
        # plt.show()
   ...
   ### Runge-Kutta sub-steps
    for rk in range( 0, rk_order ):
        ...
        ### Update boundaries
        update_boundaries( sos_field, rho_field, rhou_field, rhov_field, rhow_field, rhoE_field, u_field, v_field, w_field, P_field, T_field, mesh )
